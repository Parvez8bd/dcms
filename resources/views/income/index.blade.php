    ...
    <div class="container">
        ...
        <div class="border-0 card">
            ...
            <div class="p-0 mt-4 mb-3 border-0 card-header d-md-flex align-items-center d-block">
                <!-- page title -->
                <div>
                    <h4 class="main-title">Others Income</h4>
                    <p><small>About {{ $incomes->total() }} results found.</small></p>
                </div>
                ...
                <!-- refresh -->
                <a href="{{ route('income.index') }}" class="btn top-icon-button print-none" title="Refresh">
                ...
                <!-- add -->
                <a href="{{ route('income.create') }}" class="btn top-icon-button print-none" title="New investment">
                    ...
                    <table class="table custom-table table-hover">
                        ...
                        <tbody>
                            @forelse($incomes as $key => $income)
                                <tr>
                                    <th scope="row">{{ $incomes->firstItem() + $key }}.</th>
                                    <td>{{ $income->income_sourse }}</td>
                                    <td>{{ $income->income_date->format('d M, Y') }} </td>
                                    <td>{{ $income->note }}</td>
                                    <td class="text-end">{{ number_format($income->amount, 2) }}</td>
                                    <td class="print-none text-end">
                                       
                                        {{-- <a  href="{{ route('income.show', $income->id) }}" class="btn table-small-button" title="View" target="_blank">
                                            <i class="bi bi-eye"></i> 
                                        </a> --}}
                                    
                                        <a  href="{{ route('income.edit', $income->id) }}" class="btn table-small-button text-success" title="Update"><i class="bi bi-pencil-square"></i>
                                        </a>
                                    
                                        
                                        <span data-bs-toggle="tooltip" title="Trash">
                                            <a href="#" class="btn table-small-button text-danger" onclick="if(confirm('Are you sure want to delete?')) { document.getElementById('investment-delete-{{ $income->id }}').submit() } return false ">
                                                <i class="bi bi-trash"></i>
                                            </a>
                                        </span>

                                        <form action="{{ route('income.destroy', $income->id) }}" method="post" class="d-none" id="investment-delete-{{ $income->id }}">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <th class="text-end">{{ number_format($incomes->sum('amount'), 2) }}</th>
                                <th colspan="4">&nbsp;</th>
                            </tr>
                        </tfoot>
                        @endif
