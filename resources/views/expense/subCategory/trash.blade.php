    ...
    <div class="container">
        ...
        <div class="border-0 card">
            ...
            <div class="p-0 mb-3 border-0 card-header d-md-flex align-items-center d-block">
                <!-- page title -->
                <div class="mt-3 mb-2">
                    <h4 class="main-title">Trash Expenses Subcategory</h4>
                    <p><small>Expenses Subcategory about <strong> {{ count($records) }}</strong> result found.</small></p>
                </div>
            ...
            <div class="p-0 card-body">
                ...
                <div class="mb-3 d-block">
                    <form action="{{ route('expense-subcategory.trash') }}" method="POST">
                        ...
                        <!-- submit button -->
                        <div class="mt-4">
                            <button class="btn btn-success btn-sm" name="restore" value="1" onclick="return confirm('Do you want to restore all selected record(s)?')" {{ ($records->count() > 0) ? '' : 'disabled'}}>{{__('contents.restore_all')}}</button>

                            <button class="btn btn-danger btn-sm" name="delete" value="1" onclick="return confirm('The selected record(s) will be delete permanently!')" {{ ($records->count() > 0) ? '' : 'disabled'}}>{{__('contents.permanent_delete_all')}}</button>
                        </div>
                        <!-- submit button end -->
                    </form>
                </div>
