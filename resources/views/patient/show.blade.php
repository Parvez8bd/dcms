...
<x-app-layout>
    ...
    <div class="container">
        ...
        <div class="print-header print-show">
            <img src="{{ asset('images/logos/letterHead.png') }}" alt="Hospital Letterhead">
            ...
            <div class="mb-5 border-0 card">
                ...
                <div class="p-0 card-body ">
                    ...
                    <div class="row mt-2  print-show">
                        ...
                        <h6 class="mb-2 text-end" style="position: relative"><span
                                class="fst-italic text-muted">{{ $patient->created_at->format('d F, Y g:i A') }}</span>
                        </h6>
                        <h6 class="text-end" style="position: relative">
                            @php
                    ...
                    <div class="mb-2">
                        <div class="text-start">
                            <p class="mb-1"><b>Patient name: </b><span
                                    class="fst-italic text-muted">{{ $patient->name }}</span></p>
                            <div class="mb-1 d-flex justify-content-between">

                                <div>
                                    <b>Age: </b><span
                                        class="fst-italic text-muted">{{ $patient->age_year > 0 ? $patient->age_year . ' Years' : '' }}
                                        {{ $patient->age_month > 0 ? $patient->age_month . ' Months' : '' }}
                                        {{ $patient->age_day > 0 ? $patient->age_day . ' Days' : '' }} </span>
                                </div>
                                <div>
                                    <b>Gender: </b><span class="fst-italic text-muted">{{ $patient->gender }}</span>
                                </div>
                                <div><b>Phone: </b><span class="fst-italic text-muted">{{ $patient->phone }}
                                    </span>
                                </div>
                            </div>
                            <div class="mb-1 d-flex justify-content-between">
                                <div><b>Address: </b><span class="fst-italic text-muted">{{ $patient->address }}</span>
                                </div>
                                ...
                                @php
                                    ...
                                    $reportDelivery = $patient->created_at
                                        ->copy()
                                        ->addHours($longTestTime)
                                        ->format('d F, Y g:i A');
                                @endphp
                                @if ($longTestTime > 0)
                                    <div>
                                        <b>Report at: </b><span
                                            class="fst-italic text-muted">{{ $reportDelivery }}</span>
                                    </div>
                                @endif
                            </div>
                            <div class="mb-1 d-flex justify-content-between">
                                <p><b>Refferr Doctor: </b><span
                                        class="fst-italic text-muted">{{ $patient->patient_refferr?->doctor->owner_name ?? '' }}</span>
                                </p>
                                <p class="print-none"><b>Invoice no: </b><span
                                        class="fst-italic text-muted">{{ $patient->invoice?->invoice_no }}</span></p>
                            </div>

                        </div>
                        {{-- <div class="">
                           
                            <h6 class="mb-2"><b>Phone: </b><span class="fst-italic text-muted">{{ $patient->phone }}
                                </span></h6>
                            <h6 class="mb-2"><b>Age: </b><span
                                    class="fst-italic text-muted">{{ $patient->age_year > 0 ? $patient->age_year . ' Years' : '' }}
                                    {{ $patient->age_month > 0 ? $patient->age_month . ' Months' : '' }}
                                    {{ $patient->age_day > 0 ? $patient->age_day . ' Days' : '' }} </span></h6>
                            <h6 class="mb-2"><b>Gender: </b><span
                                    class="fst-italic text-muted">{{ $patient->gender }}</span></h6>
                        </div>
                        <div class="text-end">
                            ...
                            <h6 class="mb-2 print-none"><b>Invoice no: </b><span
                                    class="fst-italic text-muted">{{ $patient->invoice->invoice_no ?? '' }}</span></h6>
                            <h6 class="mb-2"><b>Date: </b><span
                                    class="fst-italic text-muted">{{ $patient->created_at->format('d F, Y g:i A') }}</span>
                            </h6>
                        </div> --}}
                    </div>
                    ...
                    <div class="d-flex  justify-content-between pt-5">
                        <p class="border-top px-2 ms-5 print-show"></p>
