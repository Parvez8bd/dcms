<x-app-layout>
    <x-slot name="title">Test Group</x-slot>
    ...
    <div class="container">
        ...
        <div class="border-0 card">
            ...
            <div class="p-0 mt-4 mb-3 border-0 card-header d-md-flex align-items-center d-block">
                <!-- page title -->
                <div>
                    <h4 class="main-title">Test Group</h4>
                    <p><small>About {{ $testGroups->total() }} results found.</small></p>
                </div>
                ...
                <!-- refresh -->
                <a href="{{ route('test-group.index') }}" class="btn top-icon-button print-none" title="Refresh">
                ...
                <!-- add -->
                <a href="{{ route('test-group.create') }}" class="btn top-icon-button print-none" title="Create new category">
            ...
            <div class="p-0 card-body">
                ...
                <div class="collapse print-none {{ request()->search ? 'show' : '' }}" id="category-search">
                    <div class="px-0 border-0 card card-body rounded-0">
                        <!-- search form -->
                        <form action="{{ route('test-group.index') }}" method="GET">
                            <input type="hidden" name="search" value="1">

                            <div class="row gy-1 gx-3">
                                <div class="col-12 col-sm-6 col-lg-3">
                                    <label for="project-title" class="form-label">Group name</label>
                ...
                <div class="mb-3 table-responsive">
                    <table class="table custom-table table-hover">
                        ...
                        <tbody>
                            @forelse($testGroups as $key => $record)
                                <tr>
                                    <th scope="row">{{ $testGroups->firstItem() + $key }}.</th>
                                    <td>{{ $record->title }}</td>
                                    ...
                                    <td class="print-none text-end">
                                        ...
                                        <a href="{{ route('test-group.edit', $record->id) }}" class="btn table-small-button text-success" title="Update"><i class="bi bi-pencil-square"></i>
                                        </a>
                                        ...
                                        <form action="{{ route('test-group.destroy', $record->id) }}" method="post" class="d-none" id="category-delete-{{ $record->id }}">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </td>
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <th class="text-end">{{ number_format($testGroups->sum('budget'), 2) }}</th>
                                <th>&nbsp;</th>
                            </tr>
                        </tfoot>
                        @endif --}}
