        ...
        <div class="border-0 card">
            <div class="p-0 mt-4 mb-3 border-0 card-header d-flex">
                ...
                <div>
                    ...
                    <p><small>Can All <strong>Sample Collection</strong> amount from here.</small></p>
                </div>

                <!-- header icon -->
                <a href="{{ route('sample-collection.index') }}" title="Go back" class="mt-3 mb-2 pe-0 ms-auto print-none top-icon-button">
                    <i class="bi bi-arrow-left"></i>
                </a>
            </div>

            <div class="p-0 pt-3 card-body">
                <form action="{{ route('sample-collection.create') }}" method="GET">
                ...
                <div class="p-0 card-body ">
                    ...
                    <div class="mb-2 d-flex justify-content-between">
                        ...
                        <div class="">
                            ...
                            <h6 class="mb-2"><b>Age: </b><span class="fst-italic text-muted">{{ $invoice->patient->age_year > 0? $invoice->patient->age_year. ' Years': ''}}  {{ $invoice->patient->age_month > 0? $invoice->patient->age_month. ' Months': ''}}  {{ $invoice->patient->age_day > 0? $invoice->patient->age_day. ' Days': ''}} </span></h6>
                    ...
                    <div class="mb-3 table-responsive">
                        ...
                        <form action="{{ route('sample-collection.store') }}" method="POST">
                            ...
                            <table class="table custom-table table-hover">
                                ...
                                <tbody>
                                    @forelse($invoice->report as $invoice_detail)
                                        <tr>
                                            <td scope="row">{{ $loop->iteration }}.</td>
                                            <td>{{ $invoice_detail->test->title}} 
                                                <input type="text" hidden name="invoice_id" value="{{ $invoice_detail->invoice_id }}">
                                            </td>
                                            <td class="text-end" style="width: 20%">
                                                <select name="status" id="" class="form-control">
                                                    <option selected disabled>---</option>
                                                    <option value="Painding" {{$invoice_detail->status == 'Painding' ? 'selected':'' }}>Painding</option>
