    ...
    <div class="container">
        ...
        <div class="border-0 card">
            <div class="p-0 mt-4 mb-3 border-0 card-header d-flex">
                <!-- page title -->
                <div>
                    <h4 class="main-title">Update sub category</h4>
                    <p><small>Can Update <strong>sub category</strong> from here.</small></p>
                </div>
            ...
            <div class="p-0 pt-3 card-body">
                <form action="{{ route('sub-category.update',$record->id) }}" method="POST">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-md-5">
                            <select type="text" name="category_id"  class="form-control" id="category_id" required>
                                ...
                                <option value="{{$item->id}}" {{($item->id == $record->category_id)?'selected':''}}>{{$item->title}}</option>
                                @endforeach
