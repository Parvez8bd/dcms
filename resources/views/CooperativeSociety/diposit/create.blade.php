    ...
    <div class="container">
        ...
        <div class="border-0 card">
            <div class="p-0 mt-4 mb-3 border-0 card-header d-flex">
                <!-- page title -->
                <div>
                    <h4 class="main-title">Create new diposit</h4>
                    <p><small>Create form can bring (something new) into existence.</small></p>
                </div>

                <!-- header icon -->
                <a href="{{ route('diposit.index') }}" title="Go back" class="mt-3 mb-2 pe-0 ms-auto print-none top-icon-button">
                    <i class="bi bi-arrow-left"></i>
                </a>
            </div>

            <div class="p-0 pt-3 card-body">
                <form action="{{ route('diposit.store') }}" method="POST">
                    @csrf

                    <div class="mb-3 row">
                        <div class="col-md-2">
                            <label for="account_id" class="mt-1 form-label required">Account</label>
                        </div>

                        <div class="col-md-7">
                                     
                            <select name="account_id" class="form-control select2" id="account_id" autofocus required>
                                <option value="" selected disabled>---</option>
                                    @foreach ($account as $value)
                                       
                                       <option value="{{ $value->id }}" {{ (old('account_id') == $value->id) ? 'selected' : '' }} {{ $value->is_active == 0 ? 'disabled' : '' }}>
                                        {{ $value->account_number  }} ({{ $value->contact->owner_name?? '' ?? '' }}) --
                                        {{ $value->category->title }} acount --- BDT {{ $value->amount}}/{{ $value->sub_category->title }}
                                       </option> 
                            ...
                            <select name="nomine_id" class="form-control select2" id="nomine_id" required>
                                <option value="" selected disabled>---</option>
                                    @foreach ($contacts as $contact)
                                        ...
                                        @endphp
                                       <option value="{{ $contact->id }}" {{ (old('nomine_id') == $contact->id) ? 'selected' : '' }}>{{ $contact->owner_name?? '' }} 
                                        ({{ $_contact['contact'] ?? '' }})
                                       </option> 
                                    @endforeach
                            </select>
                        ...
                        <div class="col-md-4">
                            <select name="category_id" class="form-control" id="category_id" required>
                                ...
                                @foreach ($category as $record)
                                    <option value="{{ $record->id }}" {{ (old('category_id') == $record->id) ? 'selected' : '' }}>{{ $record->title }}</option>
                                @endforeach
                            </select> 
                    ...
                    <div class="mb-3 row">
                        <div class="col-md-2">
                            <label for="sub_category_id" class="mt-1 form-label required">Diposite type</label>
                        </div>
                        ...
                        <div class="col-md-4">
                            <select name="sub_category_id" class="form-control" id="sub_category_id" required>
                                ...
                                @foreach ($subcategory as $record)
                                    <option value="{{ $record->id }}" {{ (old('sub_category_id') == $record->id) ? 'selected' : '' }}>{{ $record->title }}</option>
                                @endforeach
                            </select> 
                    ...
                    <div class="mb-3 row">
                        <div class="col-md-2">
                            <label for="diposit_date" class="mt-1 form-label required">Diposite date</label>
                        </div>

                        <div class="col-md-3">
                            <div class="input-group">
                                <input type="date" name="diposit_date" value="{{old('diposit_date', date('Y-m-d'))}}" class="form-control" id="diposit_date" required>
                            </div>
                            ...
                            <div class="input-group">
                                <select name="duration" class="form-control" id="withdraw-policy" aria-describedby="withdraw-addon" required>
                                    ...
                                    @foreach ($duration as $key => $value)
                                        <option value="{{ $key }}" {{ (old('duration') == $key) ? 'selected' : '' }}>{{ $value }}</option>
                                    @endforeach
                                </select>
                            </div>
